...
app.post("/EditCost",(req,res)=>{
    db.collection("product").findOneAndUpdate({pid:req.body.pid},{$set:{pcost:req.body.pcost}},{sort :{ _id:-1}},(err,result)=>{
        //console.log(result);
        if(err)
        return console.log(err);
        res.redirect("/"); 
...
app.post("/RemoveProduct",(req,res)=>{
   // console.log(typeof(req.body.pid));
    db.collection("product").findOneAndDelete({pid:req.body.pid},(err,result)=>{
        //console.log(result);
        if(err)
        return console.log(err);
        res.redirect("/"); 
...
app.post("/UpdateStock",(req,res)=>{
    ...
    db.collection("product").find().toArray((err,result)=>{
       ...
       // console.log((parseInt(s)+parseInt(req.body.pstock)).toString());
        db.collection("product").findOneAndUpdate({"pid":req.body.pid},{
